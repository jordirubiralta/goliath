...
class GetProductTransactionsUseCase(
    ...
    fun execute(sku: String, onSuccess: (List<Transaction>) -> Unit, onEmpty: () -> Unit, onError: (Throwable) -> Unit) {
        super.execute(onSuccess, onEmpty, onError, maybe = Maybe.defer { repository.getProductTransactionsFromDB(sku) })
    }
}